	...
	class Party extends Collection {
		...
		*/
		public function get($pdo, $id, $tableName) {
			$statement = $pdo->prepare("SELECT * FROM pokemon AS p LEFT JOIN pokemonLookup AS pl ON p.pokemonID = pl.id WHERE p.id = ? AND partyPosition < 6 ORDER BY partyPosition ASC");
